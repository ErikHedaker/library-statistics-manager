class StatisticsManager {
    constructor(errandsRaw) {
        [this.valid, this.invalid] = Errand.validator(errandsRaw.toSorted((a, b) => a.date > b.date));
        this.indent = new Indentation().next();
        this.children = StatisticsManager.createChildren(this.valid, this.indent, {
            total: errandsRaw.length,
    ...
    toString() {
        ...
        return multiline`
            ...
            ${current}valid.length: [${this.valid.length}],
            ${current}invalid.length: [${this.invalid.length}],
    ...
    getContext() {
        ...
        const joined = contexts.map(context => context.grid).reduce(
            (acc, grid) => acc.concat(GridUtility.spacers()).concat(grid)
        );
        return {
            grid: GridUtility.normalize(joined),
            funcs: contexts.reduce(
                ({ bound, offset }, { grid, funcs }) => ({
                    bound: bound.concat(funcs.map(func => (row, col) => func(row + offset, col))),
                    offset: offset + GridUtility.spacing() + GridUtility.info(grid).height,
                }), { bound: [], offset: 0 }
            ).bound,
        };
    }
    ...
    static createChildren(errands, indent, lengths) {
        const StatCluster = StatClusterBase.bind(null, indent.next().next());
        ...
        */
        ...
        const invokeProp = (key) => (obj) => typeof obj[key] === `function` ? obj[key]() : obj[key];
        const invoke = {
